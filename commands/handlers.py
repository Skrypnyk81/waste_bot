...
from config.waste_schedules import (
    WASTE_INSTRUCTIONS,
    WASTE_EMOJI,
    DAY_NAMES,
    MONTH_NAMES,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    welcome_message = (
        ...
        "Benvenuto al bot per la raccolta differenziata di Calvenzano.\n\n"
        "Questo bot ti invierà notifiche sui giorni di raccolta dei rifiuti "
        "in base al calendario 2025 del Comune di Calvenzano.\n\n"
...
async def handle_custom_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if 0 <= hours <= 23 and 0 <= minutes <= 59:
            ...
            db.set_notification_time(user_id, notification_time)
            await update.message.reply_text(f"Notifiche impostate per le {notification_time}.")
        else:
...
async def set_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "yes_address":
        await query.edit_message_text("Per favore, invia il tuo indirizzo (via e numero civico)")
        return SETTING_ADDRESS
...
async def check_today(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if waste_types:
        ...
        no_collection_message = (
            f"📅 Oggi, {DAY_NAMES[today.weekday()]} {today.day} {MONTH_NAMES[today.month]}, "
            "non è prevista alcuna raccolta di rifiuti."
        )
        await update.message.reply_text(no_collection_message)
...
async def check_tomorrow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if waste_types:
        waste_list = "\n".join([f"{WASTE_EMOJI[waste_type]} {waste_type}" for waste_type in waste_types])
        message = (
            f"📅 Domani, {DAY_NAMES[tomorrow.weekday()]} {tomorrow.day} {MONTH_NAMES[tomorrow.month]}, "
            "verranno raccolti:\n\n"
            f"{waste_list}\n\n"
...
async def stop_notifications(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Notifiche disattivate. Usa /start per riattivarle.")
...
async def restart_notifications(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Notifiche riattivate. Riceverai informazioni sulla raccolta differenziata.")
