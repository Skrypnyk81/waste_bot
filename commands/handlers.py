...
async def show_bins_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Mostra un menu interattivo per il tracciamento dei bidoni
    con pulsanti e lo stato attuale.
    """
    user = update.effective_user
    ...
    try:
        ...
        # È una buona pratica gestire possibili errori del database
    ...
    # Recupera i dati con valori di default se non presenti
    count_bins: int = user_data.get('count_bins', 0)
    limit_bins: int = user_data.get('limit_bins', "non impostato")

    # Messaggio di stato chiaro e pulito
    text = (
        f"Ciao *{user.first_name}*! 👋\n\n"
    ...
    # Creazione della tastiera con pulsanti (Inline Keyboard)
    # Ogni bottone ha un `callback_data` univoco che verrà gestito da un'altra funzione.
    keyboard = [
        [
    ...
    # Invia il messaggio con il testo e la tastiera
    # Usiamo ParseMode.MARKDOWN_V2 per formattare il testo (es. grassetto con *)
    await update.message.reply_text(
        text,
        reply_markup=reply_markup,
